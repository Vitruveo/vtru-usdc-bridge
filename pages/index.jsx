...
export default function Home(props) {
  ...
  const executeBridge = async () => {
    setLoading(true);
    try {
      const amount = Number(vtruCoinValue) * 10**18;
      if (currentFrom === VITRUVEO) {
        if (isMismatched) {
          await switchChain(VITRUVEO_CHAIN.chainId);
        }

        await bridgeVitruveoCoinToToken({ args: [BINANCE_CHAIN.chainId], overrides: { value: BigInt(amount) }});
        ...
        if (Number(vtruBinanceTokenAllowance) < amount) {
          await approveBinanceVtruSpending({ args: [BINANCE_VTRU_TOKEN_CONTRACT, BigInt(amount)] });
        }
        await bridgeBinanceTokenToCoin({ args: [VITRUVEO_CHAIN.chainId, BigInt(amount)] });
  ...
  return (
    <>
      <Head>
        <title>Vitruveo VTRU Bridge</title>
      ...
      >
              <h2 style={{fontSize: '24px', fontWeight: 600, margin: 'auto', marginBottom: '20px'}}>Vitruveo VTRU Bridge</h2>
